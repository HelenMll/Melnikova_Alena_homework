...
while massive > 0:
    ...
    massive -= 1  # повторяем все на пункт ниже, пока список не кончится
...
while massive > 0:
    ...
    while mass_number > 0:  # проходим по каждому символу
        ...
        pieces_sum += piece  # суммируем каждый элемент числа между собой
        mass_number -= 1  # опускаемся на число ниже
    if pieces_sum % 7 == 0:  # если сумма чисел числа делется без остатка
        massive -= 1  # опускаемся ниже
    else:  # иначе
        del odds[massive - 1]  # удаляем число-основу
        massive -= 1  # опускаемся ниже
